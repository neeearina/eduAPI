...
def get_all_orgs(limit: int = 10):
    ...
    with session.Session() as my_session:
        orgs = (my_session.query(session.Organizations)
                .limit(limit).all())
        if not orgs:
...
def create_org(org_info: shemas.OrganizationBase,
    ...
    with session.Session() as my_session:
        ...
        my_session.commit()
        return (my_session.query(session.Organizations)
...
def get_org_by_id(org_id: int):
    ...
    with session.Session() as my_session:
        org = (my_session.query(session.Organizations)
...
def put_org_by_id(org_id: int, update_org_info: shemas.OrganizationBase,
    ...
    with session.Session() as my_session:
        org_query = (my_session.query(session.Organizations)
                     .filter_by(id=org_id))
        org = org_query.first()
        if not org:
            raise fastapi.HTTPException(
